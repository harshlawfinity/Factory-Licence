  ...
  return (
    ...
    >
      <div className="flex items-center justify-between max-w-7xl mx-auto h-full">
        <Link to="/" className="text-2xl font-semibold text-[#642bd5]">
          <FL />
        </Link>
            ...
            >
              ...
              {item.subNav && activeDropdown === i && (
                <div className="absolute left-0 mt-2 z-50">
                  {/* Triangle Pointer */}
                  <div className="absolute top-0 left-4 w-3 h-3 bg-  rotate-45 shadow-md z-[-1] bg-gray-200"></div>
                  ...
                  <div className="mt-1  shadow-lg text-black rounded-md  bg-gray-200">
                    {item.subNav.map((subItem, j) => (
                      <Link
                        key={j}
                        to={subItem.path}
                        className="block border-b-1 px-2 border-zinc-400 px-8 py-4 text-sm text-gray-700 hover:bg-gray-100 hover:text-[#7A3EF2] whitespace-nowrap"
                      >
                        {subItem.name}
                      </Link>
                    ))}
        ...
        <div className="hidden md:flex items-center gap-4">
          <button className="border-2 px-5 py-2 flex items-center gap-2 rounded-full font-semibold text-black border-black hover:bg-gray-100 transition">
...
>
        ...
        <nav className="flex flex-col p-6 gap-6 text-gray-800 text-lg font-medium">
          {navItems.map((item, i) => (
            <div key={i} className="flex flex-col">
              ...
              {item.subNav && expandedMenus[i] && (
                <div className="ml-4  mt-2 flex flex-col gap-2 text-base text-gray-600">
                  {item.subNav.map((subItem, j) => (
                    <Link
                      key={j}
                      to={subItem.path}
          ...
          <button className="mt-6 flex items-center gap-2 border border-black px-4 py-2 rounded-full">
